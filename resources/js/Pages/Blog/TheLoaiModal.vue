                ...
                <div class="col-span-3 sm:col-span-2">
                  ...
                  <div class="mt-1 flex rounded-md shadow-sm">
                    ...
                    <input
                      ...
                      :class="errors.Ten ? 'border-red-500' :'' "
                      id="company_website"
                      class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300"
                    />
                  </div>
                  <p class="text-red-500 text-xs italic" v-if="errors.Ten">{{ errors.Ten }}</p>
                </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      this.form.post(this.route("theloai.store"), {
        preserveState: true,
    ...
    update() {
      this.form.put(this.route("theloai.update", this.form.id), {
        preserveState: true,
        onError: errors => {
          if (Object.keys(errors).length > 0) {
            this.showModal = true;
